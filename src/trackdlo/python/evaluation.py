...
class TrackDLOEvaluator:
    ...
    def get_piecewise_curve(self,Y):
        ...
        res = PWLF.fitfast(n_seg)
        xHat = np.linspace(min(x), max(x), num=50)
        yHat = PWLF.predict(xHat)
        fig, ax0 = plt.subplots(1, 1)
        ax0.scatter(x, y, color='r')
        ax0.scatter(xHat, yHat, color='g', zorder=5)
        plt.savefig("/home/hollydinkel/rmdlo_tracking/src/trackdlo/data/output/piecewise_linear.png")
